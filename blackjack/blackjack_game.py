...
class Play:
    ...
    def stand(self):
        ...
        if self.player.value > self.dealer.value:
            game_finish("You won!", 500, 250, red)
            pygame.time.delay(4000)
            self.play_or_exit()
...
while running:
    ...
    button("Hit", 30, 170, 150, 50, light_slat, dark_slat, play_blackjack.hit)
    button("Stand", 30, 240, 150, 50, light_slat, dark_slat, play_blackjack.stand)
    button("EXIT", 30, 310, 150, 50, light_slat, dark_slat, play_blackjack.exit)
